...
class VoteModel:
    ...
    def up_vote(self, pID):
        ...
        if id_check:
            try:
                ...
                return {'message':'Could not upvote for {pID} you entered!'}
        else:
    ...
    def down_vote(self, pID):
        ...
        if id_check:
            try:
                ...
                self.conn.execute(query)
                return {'message': 'Downvote for {pID} is updated!'}
                self.conn.commit()
            except:
                return {'message':'Could not downvote for {pID} you entered!'}
        else:
            return {'message': f'Post not found!'}             
        self.conn.close()
    ...
    def retrieve_votes(self, pID):
        ...
        if id_check:
            try: 
                query = f'SELECT title, up_vote, down_vote, abs(up_vote - down_vote) AS total_vote FROM Post WHERE pID = "{pID}";'
    ...
    def list_top_posts_byvotes(self, n):
        self.conn = sqlite3.connect('../service.db', isolation_level=None)
        query = 'SELECT title, up_vote, down_vote, abs(up_vote - down_vote) AS total_vote FROM Post ORDER BY abs(up_vote - down_vote) DESC;'
        result = self.conn.execute(query).fetchall()
        return jsonify(list(result))
        self.conn.close()
    ...
    def get_list(self, title):
        self.conn = sqlite3.connect('../service.db', isolation_level=None)
        query = f'SELECT title, up_vote, down_vote, abs(up_vote - down_vote) AS total_vote FROM Post WHERE title LIKE "{title}" ORDER BY (up_vote - down_vote) DESC;'
        result = self.conn.execute(query).fetchall()
        return jsonify(list(result))
        self.conn.close()