...
class VoteModel:
    ...
    def retrieve_votes(self, pID):
        self.conn = sqlite3.connect('../service.db', isolation_level=None)
        query = f'SELECT title, up_vote, down_vote FROM Post WHERE pID = "{pID}";'
    ...
    def list_top_posts_byvotes(self, n):
        self.conn = sqlite3.connect('../service.db', isolation_level=None)
        query = 'SELECT title, up_vote, down_vote FROM Post ORDER BY abs(up_vote - down_vote) DESC;'
        result = self.conn.execute(query).fetchall()
        return jsonify(list(result))
        self.conn.close()
    ...
    def get_list(self, title):
        self.conn = sqlite3.connect('../service.db', isolation_level=None)
        query = f'SELECT title, up_vote, down_vote FROM Post WHERE title LIKE "{title}" ORDER BY (up_vote - down_vote) DESC;'
        result = self.conn.execute(query).fetchall()
        return jsonify(list(result))
        self.conn.close()