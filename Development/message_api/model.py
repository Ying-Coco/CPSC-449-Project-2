...
#curl -i -X POST -H 'Content-Type:application/json' -d 
    def send_msg(self, user_fr, user_to, msg_desc, msg_flag):
        self.conn = sqlite3.connect('../service.db', isolation_level=None)
        query = f"INSERT INTO {self.table_name} (user_fr, user_to, msg_desc, msg_flag) VALUES ((SELECT username FROM User WHERE username={user_fr}), (SELECT username FROM User WHERE username={user_to}), {msg_desc}, {msg_flag})"
        try: 
            self.conn.execute(query)
...
#curl -i -X POST -H 'Content-Type:application/json' http://localhose:2015/msgs/fav_msg?msg_id=1
    def fav_msg(self, msg_id):
        self.conn = sqlite3.connect('../service.db', isolation_level=None)
        query = f'UPDATE {self.table_name} SET msg_favorite = Y WHERE msg_id={msg_id}'
        try:
            self.conn.execute(query)
